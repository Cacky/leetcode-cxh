...
public class Sort {
    ...
    public static void heapSort(int[] a) {  //堆排序
        int n = a.length, i, j;
        for (i = n / 2 - 1; i >= 0; i--) {   //建堆，从n/2开始向上调整
            adjustHeap(a, i, n);
        }
        for (j = n - 1; j >= 0; j--) {
    ...
    public static void adjustHeap(int[] a, int s, int n) {  //由于根节点编号是从0开始的，所以其左子树是2*i+1
        for (int i = s; i < n; ) {
            int max = i;
    ...
    public static void shellSort(int a[]) {//希尔排序
        ...
        while (true) {
            ...
            md = (int) d;
            for (x = 0; x < md; x++) {//对被增量分割的每小组进行直接插入排序
                for (i = x + md; i < a.length; i += md) {
                    temp = a[i];
                    for (j = i - md; j >= 0 && temp < a[j]; j -= md) {
                        a[j + md] = a[j];
                    }
                    a[j + md] = temp;
                }
